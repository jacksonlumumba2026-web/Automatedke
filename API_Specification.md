...
|---|---|---|---|
| `create_organization_with_owner` | Postgres RPC (`SECURITY DEFINER`) | Authenticated user (checked inside the function) | Atomically creates `organizations`, `organization_members` (owner), `subscriptions` (free), and default `role_permissions` |
| Send/accept/revoke invitation | Server actions + direct table access (RLS-secured) | Owner/Admin (send/revoke); token holder (accept) | Deferred past Milestone 2's scope — see `Phase1_Architecture.md` Step 6 |
